...
@Component({
  selector: 'app-aml-kyc-related-complaints',
  templateUrl: './aml-kyc-related-complaints.component.html',
  styleUrls: ['./aml-kyc-related-complaints.component.scss']
})
export class AMLKYCRelatedComplaintsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table C13: AML-KYC Related Complaints ";
    ...
    if (this.role.ROLE_CUSTCARE_C13_AMLKYC_COMPLAINTS_MAKER) {
      this.isMaker = true;
    }

    if (this.role.ROLE_CUSTCARE_C13_AMLKYC_COMPLAINTS_CHECKER) {
      this.isChecker = true;
    }
  ...
  prepareTable(){
    ...
    this.tableNames = [
      { title: "complDuringQtr", name: "No of complaints received during the quarter",inpType: "number",editable: true},
      { title: "outstdComplAtQtrEnd", name: "No of complaints outstanding at the end of the quarter ",inpType: "number",editable: true},
      { title: "outstdComplAfter30Days", name: "No of complaints outstanding with ageing status more than 30 days",inpType: "number",editable: true},
      { title: "closedComplDuringQtr", name: "No of complaints closed during the quarter",inpType: "number",editable: true},
      { title: "avgDaysToCloseComplDuringYear", name: "Average no of days taken to close the complaint (the data for complaints closed during the year may be considered)",inpType: "number",editable: true},
      { title: "highestDaysToCloseComplDuringYear", name: "Highest no of days taken to close the complaint (the data for complaints closed during the year may be considered)",inpType: "number",editable: true},
      { title: "lowestDaysToCloseComplDuringYear", name: "Lowest no of days taken to close the complaint(the data for complaints closed during the year may be considered)",inpType: "number",editable: true},
      { title: "closedComplAfter30Days", name: "No of complaints which took more than 30 days for closure",inpType: "number",editable: true},
      { title: "lapseCases", name: "No of cases where deficiency/lapse was detected",inpType: "number",editable: true},
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_AML_KYC_COMPLAINTS;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_AML_KYC_COMPLAINTS;
          this.httpService
...
getData(val) {
  this.httpService.getData(PATH.GET_AML_KYC_COMPLAINTS + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
    this.display = res;  
      ...
      this.isView = true;
        ...
        if (this.isMaker) {
          ...
          let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
             this.httpService.getData(PATH.GET_AML_KYC_COMPLAINTS + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
               if (res.length > 0) {
                 this.display = res;
                 this.isRes=true;
                 this.submitted=false;
  ...
  submitOrApproveOrREJECT(input) {
    ...
    if (submitFlag) {
      this.httpService.postData(data, PATH.POST_AML_KYC_COMPLAINTS).subscribe((res) => {
        this.submitted = true;
        let msg;
        if (input == "SUBMITTED") {
