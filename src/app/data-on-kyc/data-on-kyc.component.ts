...
@Component({
  selector: 'app-data-on-kyc',
  templateUrl: './data-on-kyc.component.html',
  styleUrls: ['./data-on-kyc.component.scss']
})
export class DataOnKycComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table F1: Data on KYC/AML";
    ...
    if (this.role.ROLE_ITD_F1_KYCAML_DATA_MAKER) {
      this.isMaker = true;
    }

    if (this.role.ROLE_ITD_F1_KYCAML_DATA_CHECKER) {
      this.isChecker = true;
    }
  ...
  //   ]; 
    ...
    this.tableNames = [
      { title: "accountType", name: "accountType",inpType: "number",editable: false},
      { title: "signNotUploadedDuringQtr", name: "Number (during the quarter)",inpType: "string",editable: true},
      { title: "signNotUploadedOnQtrEnd", name: "Number (cumulative as on quarter end)",inpType: "number",editable: true},
  ...
  {
    this.display=[
      { accountType:"Customer Ids pending for deduplication" },
      { accountType:"Accounts pending for upload on CKYCR" },
      { accountType:"No of accounts required to be uploaded on CKYCR" },
      { accountType:"Savings account closed within one year of opening " },
      { accountType:"Current account closed within one year of opening " },
      { accountType:"Accounts where signatures  are not uploaded in the system" } 
    ]
  }
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_KYC_AML_DATA;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_KYC_AML_DATA;
          this.httpService
...
getData(val) {
  this.httpService.getData(PATH.GET_KYC_AML_DATA + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
    this.display = res;  
      ...
      this.isView = true;
        ...
        if (this.isMaker) {
          let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
             this.httpService.getData(PATH.GET_KYC_AML_DATA + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
               if (res.length > 0) {
                 this.display = res;
                 this.isRes=true;
                 this.submitted=false;
  ...
  submitOrApproveOrREJECT(input) {
    ...
    if (submitFlag) {
      this.httpService.postData(data, PATH.POST_KYC_AML_DATA).subscribe((res) => {
        this.submitted = true;
        let msg;
        if (input == "SUBMITTED") {
