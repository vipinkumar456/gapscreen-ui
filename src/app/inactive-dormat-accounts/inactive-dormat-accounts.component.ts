...
@Component({
  selector: 'app-inactive-dormat-accounts',
  templateUrl: './inactive-dormat-accounts.component.html',
  styleUrls: ['./inactive-dormat-accounts.component.scss']
})
export class InactiveDormatAccountsComponent implements OnInit {
  heading: string;
  display: Array<any> = [];
  submittedToAdd: any;
  dateValue: Date;
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table C4:Inactive and dormant accounts";
    ...
    if (this.role.ROLE_CAML_C4_INACTIVE_DORMANT_ACCOUNTS_MAKER) {
      this.isMaker = true;
    }

    if (this.role.ROLE_CAML_C4_INACTIVE_DORMANT_ACCOUNTS_CHECKER) {
      this.isChecker = true;
    }
  ...
  prepareTable(){
    this.tableHeadingNames = [
      { title: "inactiveDormatAccounts", name: "Inactive and dormant accounts",inpType: "number",editable: false},
      { title: "Low", name: "High risk customer",inpType: "number",editable: true},
      { title: "Medium", name: "Medium risk customer",inpType: "number",editable: true},
      { title: "High", name: "Low risk customer",inpType: "number",editable: true},
    ];

    this.tableNames = [
      { title: "accountType", name: "Inactive and dormant accounts",inpType: "number",editable: false},
      { title: "lowRiskCustomerNumber", name: "Number",inpType: "number",editable: true},
      { title: "lowRiskCustomerAmount", name: "Amount",inpType: "Amount",editable: true},
      { title: "mediumRiskCustomerNumber", name: "Number",inpType: "number",editable: true},
      { title: "mediumRiskCustomerAmount", name: "Amount",inpType: "Amount",editable: true},
      { title: "highRiskCustomerNumber", name: "Number",inpType: "number",editable: true},
      { title: "highRiskCustomerAmount", name: "Amount",inpType: "Amount",editable: true},
    ];
  ...
  {
    this.display=[
      { accountType:"3e. Accounts where STR was filed for reactivated accounts" },
      { accountType:"3f. Accounts where CTR was filed for reactivated accounts" },
    ]
  }
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_INACTIVE_DORMANT_ACCOUNTS;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_INACTIVE_DORMANT_ACCOUNTS;
          this.httpService
...
getData(val) {
  this.httpService.getData(PATH.GET_INACTIVE_DORMANT_ACCOUNTS + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
    this.display = res;  
    ...
    this.isView = true;
      ...
      if (this.isMaker) {
        let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
           this.httpService.getData(PATH.GET_INACTIVE_DORMANT_ACCOUNTS + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
             if (res.length > 0) {
               this.display = res;
               this.isRes=true;
               this.submitted=false;
...
submitOrApproveOrREJECT(input) {
  ...
  if (submitFlag) {
    this.httpService.postData(data, PATH.POST_INACTIVE_DORMANT_ACCOUNTS).subscribe((res) => {
      this.submitted = true;
      let msg;
      if (input == "SUBMITTED") {
