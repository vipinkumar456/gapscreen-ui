...
@Component({
  selector: 'app-jurisdictions-risk-profile',
  templateUrl: './jurisdictions-risk-profile.component.html',
  styleUrls: ['./jurisdictions-risk-profile.component.scss']
})
export class JurisdictionsRiskProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table B8: Jurisdictions in/with which bank's customers have presence and/or operations: Risk Profile";
    ...
    if (this.role.ROLE_NIL_DATA_B8_JURISDICTIONS_RISK_PROFILE_MAKER) {
      this.isMaker = true;
    }

    if (this.role.ROLE_NIL_DATA_B8_JURISDICTIONS_RISK_PROFILE_CHECKER) {
      this.isChecker = true;
    }
  ...
  prepareTable(){
    ...
    this.tableNames = [
      ...
      { title: "presence", name: "presence",inpType: "number",editable: true},
      { title: "operations", name: "operations",inpType: "number",editable: true},
  ...
  totalAddition(itm, name) {
    ...
    if (name.title == 'presence') {
      this.total.presence = 0;
    }
    if (name.title == 'operations') {
      this.total.operations = 0;
    }
    if (name.title == 'total') {
      this.total.total = 0;
    }    
    this.display.forEach(elm => {
      ...
      if (elm.presence && name.title == 'presence') {
        this.total.presence = this.total.presence + parseFloat(elm.presence);
      }
      if (elm.operations && name.title == 'operations') {
        this.total.operations = this.total.operations + parseFloat(elm.operations);
      }
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_JURISDICTIONS_RISK_PROFILE;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_JURISDICTIONS_RISK_PROFILE;
          this.httpService
...
getData(val) {
  this.httpService.getData(PATH.GET_JURISDICTIONS_RISK_PROFILE + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
    this.display = res;  
      ...
      this.isView = true;
        ...
        if (this.isMaker) {
          let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
             this.httpService.getData(PATH.GET_JURISDICTIONS_RISK_PROFILE + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
               if (res.length > 0) {
                 this.display = res;
                 this.isRes=true;
                 this.submitted=false;
                 this.display.forEach((elm)=>{
                   this.total.presence=this.total.presence+elm.presence;
                  this.total.operations=this.total.operations+elm.operations;
  ...
  submitOrApproveOrREJECT(input) {
    ...
    if (submitFlag) {
      this.httpService.postData(data, PATH.POST_JURISDICTIONS_RISK_PROFILE).subscribe((res) => {
        this.submitted = true;
        let msg;
        if (input == "SUBMITTED") {
