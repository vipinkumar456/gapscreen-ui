...
@Component({
  ...
  styleUrls: ['./post-transaction.component.scss']
})
export class PostTransactionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table C9:  Types of  Alerts (post-transaction)";
    ...
    if (this.role.ROLE_CAML_C9_POST_TRANSACTION_ALERTS_MAKER) {
      this.isMaker = true;
    }

    if (this.role.ROLE_CAML_C9_POST_TRANSACTION_ALERTS_CHECKER) {
      this.isChecker = true;
    }
  ...
  prepareTable(){
    ...
    this.tableHeadingNames = [
      { title: "Frequency of reviews for customers whose risk grade is  (pick from appropriate drop down options)",name: "transactions"}
    ];

    this.tableNames = [
      { title: "alertType", name: "transactions",inpType: "number",editable: false},
      { title: "noOfAlertsRaised", name: "No. of Alerts raised",inpType: "number",editable: true},
      { title: "noOfAlertsClosed", name: "No. of Alerts closed (without raising internal suspicion reports)",inpType: "number",editable: true},
      { title: "avgTimeForClosing", name: "Average time for closing the alerts (no. of days)",inpType: "number",editable: true},
      { title: "noOfIntlSuspicionReports", name: "Total no. of internal suspicion reports filed following alerts",inpType: "number",editable: true},
    ];
  ...
  {
    this.display=[
      { alertType:"Large transactions (other than cash and FX)" },
      { alertType:"Large fx transactions (Other than cash)" },
      { alertType:"Large cash transactions" },
      { alertType:"Cash-in and Cash-out transactions" },
      { alertType:"Series of linked cash transactions" },
      { alertType:"Funds-in and Funds-out transactions"},
      { alertType:"Caution listed/sanctioned customers" },
      { alertType:"Sanctioned jurisdictions" },
      { alertType:"Transactions with non equivalent jurisdictions" },
      { alertType:"Transactions related to PEPs, other high risk accounts" },
      { alertType:"Transactions related to newly reactivated accounts" },
      { alertType:"Money or value transfer services" },
      { alertType:"Transactions in accounts of minors" },
      { alertType:"All other types of transactions" },
    ]
  }
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_TRANSACTION_Alert;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_TRANSACTION_Alert;
          this.httpService
...
getData(val) {
  this.httpService.getData(PATH.GET_TRANSACTION_Alert + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
    this.display = res;  
      ...
      this.isView = true;
        ...
        if (this.isMaker) {
          let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
             this.httpService.getData(PATH.GET_TRANSACTION_Alert + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
               if (res.length > 0) {
                 this.display = res;
                 this.isRes=true;
                 this.submitted=false;
  ...
  submitOrApproveOrREJECT(input) {
    ...
    if (submitFlag) {
      this.httpService.postData(data, PATH.POST_TRANSACTION_Alert).subscribe((res) => {
        this.submitted = true;
        let msg;
        if (input == "SUBMITTED") {
