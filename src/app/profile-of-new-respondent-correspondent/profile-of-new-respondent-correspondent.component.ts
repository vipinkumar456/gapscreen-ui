...
@Component({
  selector: 'app-profile-of-new-respondent-correspondent',
  templateUrl: './profile-of-new-respondent-correspondent.component.html',
  styleUrls: ['./profile-of-new-respondent-correspondent.component.scss']
})
export class ProfileOfNewRespondentCorrespondentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table B6:  Profile of new respondent and correspondent banks";
    ...
    if (this.role.ROLE_IBD_B6_NRCB_PROFILE_MAKER) {
      this.isMaker = true;
    }

    if (this.role.ROLE_IBD_B6_NRCB_PROFILE_CHECKER) {
      this.isChecker = true;
    }
  ...
  prepareTable(){
    ...
    this.tableNames = [
      ...
      { title: "rejected", name: "No. of new respondent/correspondent banks applications rejected due to AML/CFT considerations",inpType: "number",editable: true},
    ];
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_NRCB_PROFILE_CONTROLLER;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_NRCB_PROFILE_CONTROLLER;
          this.httpService
...
getData(val) {
  this.httpService.getData(PATH.GET_NRCB_PROFILE_CONTROLLER + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
    this.display = res;   
      ...
      this.isView = true;
        ...
        if (this.isMaker) {
          let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
             this.httpService.getData(PATH.GET_NRCB_PROFILE_CONTROLLER + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
               if (res.length > 0) {
                 this.display = res;
                 this.isRes=true;
                 this.submitted=false;
  ...
  submitOrApproveOrREJECT(input) {
    ...
    if (submitFlag) {
      this.httpService.postData(data, PATH.POST_NRCB_PROFILE_CONTROLLER).subscribe((res) => {
          this.submitted = true;
        let msg;
        if (input == "SUBMITTED") {
