...
@Component({
  selector: 'app-profile-of-respondent',
  templateUrl: './profile-of-respondent.component.html',
  styleUrls: ['./profile-of-respondent.component.scss']
})

export class ProfileOfRespondentComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table B5:Profile of Respondent and Correspondent banks";
    ...
    if (this.role.ROLE_IBD_B5_RCB_PROFILE_MAKER) {
      this.isMaker = true;
    }

    if (this.role.ROLE_IBD_B5_RCB_PROFILE_CHECKER) {
      this.isChecker = true;
    }
  ...
  prepareTable(){
    ...
    this.tableNames = [
      { title: "bankType", name: "banks",inpType: "number",editable: false},
      { title: "lowRiskNoOfBanks", name:"No. of banks ",inpType: "number",editable: true},
      { title: "lowRiskTotalDebits", name: "Total debits during the quarter (amount)",inpType: "number",editable: true},
      { title: "lowRiskTotalCredits", name: "Total credits during the quarter (amount)",inpType: "number",editable: true},
      { title: "mediumRiskNoOfBanks", name: "No. of banks",inpType: "number",editable: true},
      { title: "mediumRiskTotalDebits", name: "Total debits during the quarter (amount)",inpType: "number",editable: true},
      { title: "mediumRiskTotalCredits", name: "Total credits during the quarter (amount)",inpType: "number",editable: true},
      { title: "highRiskNoOfBanks", name: "No. of banks",inpType: "number",editable: true},
      { title: "highRiskTotalDebits", name: "Total debits during the quarter (amount)",inpType: "decimal",editable: true},
      { title: "highRiskTotalCredits", name: "Total credits during the quarter (amount)",inpType: "decimal",editable: true}
    ];
  ...
  {
    this.display=[
      { bankType:"Respondent banks" },
      { bankType:"Correspondent banks" },
      { bankType:"Correspondent banks in juridisction which have strategic deficiencies or have not made sufficient progress in implemenation of FATF recommendations" },
    ]
  }
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_RCB_PROFILE_CONTROLLER;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_RCB_PROFILE_CONTROLLER;
          this.httpService
...
getData(val) {
this.httpService.getData(PATH.GET_RCB_PROFILE_CONTROLLER + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
  this.display = res;   
    ...
    this.isView = true;
      ...
      if (this.isMaker) {
        let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
           this.httpService.getData(PATH.GET_RCB_PROFILE_CONTROLLER + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
             if (res.length > 0) {
               this.display = res;
               this.isRes=true;
               this.submitted=false;
...
submitOrApproveOrREJECT(input) {
 ...
 if (submitFlag) {
   this.httpService.postData(data, PATH.POST_RCB_PROFILE_CONTROLLER).subscribe((res) => {
       this.submitted = true;
     let msg;
     if (input == "SUBMITTED") {
