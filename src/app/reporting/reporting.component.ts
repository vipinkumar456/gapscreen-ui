  ...
  ngOnInit(): void {
   ...
   this.deleteBar=true;  
    ...
    if(this.role.ROLE_EXPOSURE_QCCP_CHECKER){
     ...
     this.heading="Format for reporting of exposures to QCCPs"
    this.tableNames = [
      {
        title: "serialNo",
        name: "S.no",
  ...
  getRecords(val){
    this.httpService.getData(PATH.EXPOSURE_QCCP_GET_RECORDS+"?status="+val).subscribe((res) => {
      ...
      this.display.forEach((elm,index)=>{
        ...
        elm.totalExposure = parseInt(elm.tradeExposure) + parseInt(elm.defaultFundExposure) + parseInt(elm.others);
      })
...
if (!((/^\s*(?=.*[1-9])\d*(?:\.\d{1,2})?\s*$/g).test(totalExposure))) {
  this.messageService.add({
    severity: "error",
    summary: "Error",
...
getTotalExposure(row){
  if(row && row.tradeExposure && row.defaultFundExposure && row.others){
    row.totalExposure = parseInt(row.tradeExposure) + parseInt(row.defaultFundExposure) + parseInt(row.others);
