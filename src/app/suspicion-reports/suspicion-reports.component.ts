...
@Component({
  selector: 'app-suspicion-reports',
  templateUrl: './suspicion-reports.component.html',
  styleUrls: ['./suspicion-reports.component.scss']
})
export class SuspicionReportsComponent implements OnInit {
  heading: string;
  display: Array<any> = [];
  submittedToAdd: any;
  dateValue: Date;
  ...
  ngOnInit(): void {
    ...
    this.heading = "Table C11: Suspicion Reports";
    ...
    if (this.role.ROLE_CAML_C11_SUSPICION_REPORTS_MAKER) {
      this.isMaker = true
    }

    if (this.role.ROLE_CAML_C11_SUSPICION_REPORTS_CHECKER) {
      this.isChecker = true
  ...
  prepareTable(){
    this.tableHeadingNames = [
      { title: "Frequency of reviews for customers whose risk grade is  (pick from appropriate drop down options)",name: "frequency"}
    ];


    this.tableNames = [
      { title: "type", name: "firstData",inpType: "number",editable: false},
      { title: "value", name: "secondData",inpType: "number",editable: true}
    ];
    this.prepareReportData();
  }
  ...
  {
    this.display=[
      { type:"No. of internal suspicion reports filed with designated director",inP: "number"},
      { type:"Average time taken for raising suspicion reports", inp: "select" },
      { type:"No. of internal suspicion reports closed by designated director", inp: "number"},
      { type:"Average time taken for closing suspicion reports" ,inp: "number"},
      { type:"No. of STRs filed by the designated director" ,inp: "number"},
      { type:"Average time taken from the receipt of suspicion report till the filing of STRs" },
      { type:"Threshold amount for raising STR for reactivated Inoperative/Dormant account" },
      { type:"No of officers attending/processing STRs/CTRs" },
      { type:"No of alerts processed per employee" },
      { type:"No of alerts closed per employee" },
      { type:"Instances in which STR filing was delayed" },  
    ]
  }
  
  getDropDown() {
    let itm = 'post-facto-monitoring-avg-time';
  ...
  submit(val): void {
    ...
    if(this.isRes){
      this.confirmationService.confirm({
        ...
        accept: () => {
          var uri = PATH.UPDATE_SANCTION_CAUTION_REPORT;
            this.httpService
   ...
   this.confirmationService.confirm({
      message: "Are you sure that you want to Submit?",
      accept: () => {
        var uri = PATH.POST_SANCTION_CAUTION_REPORT;
          this.httpService
...
getData(val) {
  this.httpService.getData(PATH.GET_SANCTION_CAUTION_REPORT + "?quarterEndingDate=" + this.selectedQuarter +  "&status=" + val).subscribe((res) => {
    this.display = res;  
      ...
      this.isView = true;
        ...
        if (this.isMaker) {
          let status= this.rejectedFlag?"REJECTED":"SUBMITTED";
             this.httpService.getData(PATH.GET_SANCTION_CAUTION_REPORT + "?quarterEndingDate=" + this.selectedQuarter + "&status=" + status).subscribe((res) => {
               if (res.length > 0) {
                 this.display = res;
                 this.isRes=true;
                 this.submitted=false;
  ...
  submitOrApproveOrREJECT(input) {
    ...
    if (submitFlag) {
      this.httpService.postData(data, PATH.POST_SANCTION_CAUTION_REPORT).subscribe((res) => {
        this.submitted = true;
        let msg;
        if (input == "SUBMITTED") {
